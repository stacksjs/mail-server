...
import type { AuthCallback, AuthError, AuthOptions, CommandHandler, NoArgsHandler, SMTPConnection as SMTPConnectionInterface, SMTPServer as SMTPServerInterface } from './types'
...
type SASLHandler = (this: SMTPConnectionInterface, args: string[], callback: () => void) => void

/**
 * Creates a handler for new socket
 */
export class SMTPConnection extends EventEmitter implements SMTPConnectionInterface {
  public id: string
  public session: Session
  public name: string
  public _server: SMTPServerInterface
  ...
  constructor(server: SMTPServer, socket: Socket | TLSSocket, options: ConnectionOptions = {}) {
    ...
    this._server = server as unknown as SMTPServerInterface
    this._socket = socket
  ...
  handler_AUTH(command: Buffer, callback: () => void): void {
    ...
    handler.call(this as unknown as SMTPConnectionInterface, args.slice(1), callback)
  }
  ...
  _onAuth(auth: AuthOptions, session: Session, callback: AuthCallback): void {
    this._server.onAuth(auth, session, (err: AuthError | null, response?: any) => {
